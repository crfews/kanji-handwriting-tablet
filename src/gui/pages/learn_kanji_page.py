...
from data import KanjiCard, Drawing
from data.database import maybe_connection
from gui.widgets.writing_widgets import CharacterDrawing

class LearnKanjiWidget(QtWidgets.QWidget):

    def __init__(self, parent=None):
        super().__init__(parent)

        # State variables
        self._it: Optional[Iterator[KanjiCard]] = None
        ...
        # Main labels
        self.kanji_label = QtWidgets.QLabel("", self)
        ...
        root.addWidget(self.kanji_label)
        root.addWidget(self.romaji_label)
        root.addWidget(self.status_label)

        # add drawing widget
        self.drawing = CharacterDrawing(parent = self, on_cleared=self._on_cleared, on_submitted=self._on_submitted)
    ...
    def start(self) -> None:
        if self._it is None:
            with maybe_connection(None) as con:
                ...
                self._it = iter(list(query_learnable_kanji_cards(con)))
        self.next_card()
    ...
    def _set_done(self) -> None:
        self.kanji_label.setText("✓")
        self.romaji_label.setText("")
        self.status_label.setText("No more learnable kanji cards.")

    def _render_current(self) -> None:
        assert self._current is not None
        self.kanji_label.setText(getattr(self._current, "kanji", ""))
    ...
    def _on_submitted(self, strokes: list[list[float]]) -> None:
        ...
        print(
            f"Submitted for kanji={getattr(self._current, 'kanji', '')!r}, "
