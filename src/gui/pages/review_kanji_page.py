...
from data.queries import query_reviewable_kanji_cards
...
class ReviewKanjiPage(QtWidgets.QWidget):
    """
    Review session (Kanji):
    ...
    def __init__(self, parent=None):
        ...
        # Created lazily ONLY when we have cards
    ...
    def _load_if_needed(self) -> None:
        ...
        self.kanji_question_widget = KanjiQuestionWidget(self.stack)
        self.kanji_answer_widget = KanjiAnswerWidget(self.stack)

        self.stack.addWidget(self.kanji_question_widget)
        self.stack.addWidget(self.kanji_answer_widget)

        self.kanji_question_widget.submitted.connect(self.show_answer)
        self.kanji_answer_widget.next_signal.connect(self.next_kanji)
        self.kanji_answer_widget.try_again_signal.connect(self.try_again)
    ...
    def try_again(self) -> None:
        ...
        if not self._cards:
            self.stack.setCurrentWidget(self._nothing_here)
            return
        assert self.kanji_question_widget is not None
...
class KanjiQuestionWidget(QtWidgets.QWidget):
    ...
    def set_kanji(self, c: KanjiCard) -> None:
        # Prompt user with meaning/readings; they must produce the kanji
        meaning = c.meaning or ""
        oy = c.on_yomi or ""
        ky = c.kun_yomi or ""

        self.prompt_label.setText(meaning if meaning else "Draw the kanji")
        if oy and ky:
...
class KanjiAnswerWidget(QtWidgets.QWidget):
    ...
    def set_kanji(self, c: KanjiCard) -> None:
        self.correct_label.setText(c.kanji)
        ...
        if oy:
            parts.append(f"On: {oy}")
        if ky:
            parts.append(f"Kun: {ky}")
